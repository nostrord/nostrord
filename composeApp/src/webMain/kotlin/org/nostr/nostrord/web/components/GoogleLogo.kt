...
/**
 * The multicolor Google "G" mark, inlined so the login button needs no asset fetch.
 * Brand colors are fixed by Google's guidelines, hence the literal hexes (exempt from
 * the design-token rule, same as other brand marks).
 */
val GoogleLogo =
    FC<Props> {
        svg {
            className = ClassName("google-logo")
            viewBox = "0 0 48 48"
            ariaHidden = true
            path {
                fill = "#FFC107"
